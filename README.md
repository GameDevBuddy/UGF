...
Copy `addons/universal_gameplay/` into your project and enable **Universal Gameplay
Framework** in *Project → Project Settings → Plugins*. That adds the `EventBus` and
`FrameworkCore` autoloads and gives the framework's semantic input actions default bindings,
never overwriting one you have already defined.

Then tell it which modules you want and where your content lives. That is one resource:

```gdscript
# res://game/framework_settings.tres, authored in the inspector
definition_paths  = ["res://game/content"]
enabled_modules   = {
    &"module.entity": true,
    &"module.items": true,
    &"module.inventory": true,
    &"module.missions": true,
}
```

Name it in *Project Settings → Universal Gameplay → Config → Settings Path* and the framework
brings those modules up in dependency order on startup, scans your content, validates it, and
...
Get the list wrong and nothing starts half-built — you get an error naming the modules to add.
Every module is optional, every one is removable, and
[`docs/modules.md`](docs/modules.md) says what each needs and what it will use if present.
Two worked examples are in [`examples/`](examples/), and neither contains a line of GDScript.
...
| M19 deliverable | Where |
| --- | --- |
| Addon packaging | `plugin.cfg`, `plugin.gd`, `core/module_catalog.gd` |
| Module enablement | `core/framework_bootstrapper.gd`, `core/framework_settings.gd` |
| Example projects | `examples/adventure/`, `examples/survival/` |
| API docs | `docs/api-reference.md`, generated by `tools/doc_generator.gd` |
| Module enablement docs | `docs/modules.md`, generated from the manifests |
| Migration guide | `docs/migration-guide.md` |
| Benchmarks | `tests/cases/test_benchmarks.gd`, `tests/support/benchmark_harness.gd` |

**M19 exit gate — a new project integrates Core and chosen modules without copying
game-specific code:**

- *Settings alone bring up the modules* —
  `test_packaging.gd::test_settings_alone_bring_up_the_modules_they_ask_for`,
  `test_the_entire_framework_installs_at_once`
- *An example project is one resource and some content* —
  `test_examples.gd::test_the_adventure_example_installs_from_its_settings_alone`,
  `test_the_survival_example_installs_from_its_settings_alone`
- *No game-specific code is copied* —
  `test_examples.gd::test_the_examples_contain_no_gdscript_at_all`,
  `test_the_examples_reference_only_the_addon`
- *The addon stands alone* —
  `test_packaging.gd::test_no_addon_file_reaches_outside_the_addon`,
  `test_the_addon_ships_no_content_definitions`
- *And the content actually plays* —
  `test_examples.gd::test_the_adventure_slice_runs_end_to_end`,
  `test_the_survival_slice_crafts_from_authored_content`

```
91 suite(s), 2256 test(s), 2256 passed, 0 failed, 8981 assertions
RESULT: PASS
```

**`enabled_modules` was a list nothing read.** For eighteen milestones a project could tick
Inventory in its settings and get `is_module_enabled()` returning true and `has_feature()`
returning false forever. Every method involved behaved exactly as documented, which is why the
gap survived eighteen milestones: there was no bug to find, only a step nobody had written.
`FrameworkBootstrapper` is that step, and it runs during `bootstrap()` by default because the
alternative is a framework that greets its first user with a project that compiles, validates and
does nothing.

**The catalog is a written-out table, not a folder scan.** Scanning `addons/` works perfectly in
the editor and finds nothing at all in an exported build — `DirAccess` over `res://` sees only
what the exporter packed, and scripts are remapped on the way in. A literal table exports
correctly and can drift, so a test compares it against the files on disk and fails naming the
module that is missing from one side.

**The bootstrapper reports; it does not repair.** A settings file asking for Combat without Items
gets an error naming what to add, and nothing is registered — not even the half that resolved
(rule 17). Quietly adding the missing modules would leave a project whose settings file no longer
describes its game, and the first time one of those uninvited modules misbehaved there would be
nothing to point at.

**The examples contain no GDScript, and a test enforces it.** That is not tidiness. The exit gate
is "integrates without copying game-specific code", and one helper script in `examples/` would
fail that gate while looking like it passed. The same test forbids referencing any path outside
`addons/universal_gameplay/`, because an example that loads something from this repository's test
folder works here and breaks the moment somebody copies it into their own game.

**Benchmarks assert the shape of the curve, never the wall clock.** A CI runner shares a machine
with whatever else is on it, so "this took under 3ms" is a test that fails on a busy afternoon,
gets muted within a month, and then guards nothing. What is asserted is that a constant-time
operation measured against fifty times the data costs about the same. The tolerance is 4x —
loose enough that noise never trips it, tight enough that a lookup turned into a loop cannot hide.
Verified by pointing the population benchmark at a genuinely linear call: **49.6x measured for
50x the data.** The numbers themselves are printed in the CI log, unasserted, so a reviewer can
still see a 40% regression that no safe threshold would ever catch.

**The reference documentation is generated from the source it documents,** and a test fails when
the committed file and the source disagree. Hand-written reference tables rot silently, because
nobody reads a table looking for the row that is not in it — they read it, believe it, and act on
it. The prose that needs judgement stays hand-written and is checked for what goes stale first:
dead links, and a save-schema number that moved without anyone writing down how to migrate.
...
**`Array[StringName].sort()` does not sort alphabetically.** StringNames compare by their
address in the engine's intern table, so sorting them yields the order those names were first
created in — which depends on which scripts the engine happened to load first, and therefore
differs between the editor, a headless run and an exported build.

```gdscript
var ids: Array[StringName] = [&"module.zebra", &"module.apple", &"module.mango"]
ids.sort()   # [&"module.mango", &"module.apple", &"module.zebra"]
```

This is worse than an unsorted list, because the result is perfectly deterministic within one
process. `ModuleCatalog.resolve_order()` used it to break ties and looked stable for exactly as
long as nobody compared two builds. Anywhere the order is user-visible or has to reproduce, sort
with `str(a) < str(b)`. The test that was supposed to catch this compared two calls inside one
process and could never have failed; it now asserts the order is alphabetical, which is a
property of the ids rather than of the run.

**A test that aborts partway still reports PASS if it asserted anything first.** GDScript does
not propagate a runtime error out of the call, so the runner's zero-assertion guard only catches
a test that dies before its first assertion. A test that asserts three things and then calls a
method that does not exist records three passes and stops. What catches it is the CI step that
...
examples/                          two projects. no GDScript, by rule
├── adventure/                     talk, fetch, get credit
│   ├── settings.tres              the whole integration: modules + content path
│   └── content/                   items, a conversation, a mission
└── survival/                      gather, craft, eat
    ├── settings.tres
    └── content/                   materials, a recipe, a need, some food

docs/
├── modules.md                     generated: every module, what it needs
├── api-reference.md               generated: every class_name, one line each
├── migration-guide.md             upgrading, save schemas, replacing a module
├── implementation-plan.md         the source specification
└── ontology-rulebook.md           the rules the specification is built on

tools/
├── doc_generator.gd               builds the generated docs from the source
└── generate_docs.gd               writes them to docs/
...
├── cases/                         the suites
├── support/                       fixtures and the benchmark harness
...
All twenty milestones are done. The build order followed dependency, not feature appeal.
